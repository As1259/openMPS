...
namespace de.as1259.openMPS.MYSQLConnectionTools
{
    ...
    public static class CollectionToMysql
    {
        ...
        {
            long onlineOID = shellDT("Select value from snmp.version where Name = 'mps'").Rows[0].Field<int>("value");
            ;
            var version = CounterConfig.shellDT("Select version from INFO").Rows[0].Field<long>("OIDVersion");
            if (version > onlineOID)
        ...
        /// <summary>
        ///     To update the OID Table
        /// </summary>
        public static void updateOID()
        {
            long onlineOID = shellDT("Select value from snmp.version where Name = 'oid'").Rows[0].Field<int>("value");
            ;
            var version = CounterConfig.shellDT("Select OIDVersion from INFO").Rows[0].Field<long>("OIDVersion");
            if (version < onlineOID)

            {
                var dt = shellDT("Select * from snmp.OID");

                if (dt.Rows.Count > 0)
                {
                    CounterConfig.shell("Delete from OID");
                    CounterConfig.shell("delete from sqlite_sequence where name = 'OID';");
                    ...
                    CounterConfig.shell("Update INFO   set OIDVersion = " + onlineOID + ";");
        ...
        public static void updateOID(SQLiteConnector sqlCon)
        {
...

            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    CounterConfig.shell("Update INFO   set OIDVersion = " + fileOID + ";");
        ...
        public static void writeToExternMYSQL()
        {
            ...
            for (var i = 0; i < dt.Rows.Count; i++)
            {
                var cmd =
                    ...
                    "'," //Kundennummer
                    + CounterConfig.shellDT("Select version from INFO").Rows[0].Field<long>("version") + ","
                    + CounterConfig.shellDT("Select OIDVersion from INFO").Rows[0].Field<long>("OIDVersion") + ""
